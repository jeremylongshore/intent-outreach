    ...
    cors(req, res, async () => {
        try {
            if (req.method === 'POST' && req.path === '/keys/set') {
                ...
                try {
                    await secrets.getSecret({ name: parent });
                }
                catch {
                    await secrets.createSecret({ parent: `projects/${PROJECT}`, secretId: name, secret: { replication: { automatic: {} } } });
                }
                // add version
            ...
            if (req.method === 'GET' && req.path === '/keys/test') {
                ...
                try {
                    ...
                    const buf = acc.payload?.data;
                    const has = !!buf && buf.length > 0;
...
export const runQueuedCampaign = onDocumentCreated({ region: REGION, document: 'queues/{id}' }, async (event) => {
    ...
    try {
        ...
        const leads = camp.domains?.slice(0, 3)?.map((d, i) => ({ domain: d, contact: `user${i}@${d}`, score: 0.7 + i * 0.05 })) || [];
