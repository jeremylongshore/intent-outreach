...
const PROVIDERS = ["CLAY", "APOLLO", "CLEARBIT", "CRUNCHBASE"] as const;
...
/**
 * Result of secret check/creation
 */
 ...
 */
export type SecretStatus = "ok" | "missing" | "placeholder" | "error";
 ...
 */
export async function ensureTenantSecrets(
  tenantId: string
  ...
  const result: SecretCheckResult = {
    tenantId,
    secrets: {} as Record<Provider, SecretStatus>,
    allConfigured: false,
    missing: [],
    placeholders: [],
    errors: [],
  ...
  for (const provider of PROVIDERS) {
    ...
    try {
      ...
      result.errors.push(`${provider}: ${(error as Error).message}`);
    }
  }
...
async function ensureSecret(
  secretName: string,
  placeholderValue: string = PLACEHOLDER_VALUE
 ...
 */
async function addSecretVersion(secretName: string, value: string): Promise<void> {
 ...
 */
export async function ensureIamBinding(
  secretName: string,
  member: string,
  role: string
): Promise<void> {
...
export async function validateTenantSecrets(
  tenantId: string,
  requiredProviders: Provider[] = [...PROVIDERS]
): Promise<{ valid: boolean; missing: Provider[]; placeholders: Provider[] }> {
  console.log(`[TENANT-SECRETS] Validating secrets for tenant: ${tenantId}`);

  const missing: Provider[] = [];
  const placeholders: Provider[] = [];
 ...
 */
export async function writeSecretHealthToFirestore(
  tenantId: string,
 ...
 */
export function getTenantSecretName(tenantId: string, provider: Provider): string {
