...
import {
  ensureTenantSecrets,
  validateTenantSecrets,
  writeSecretHealthToFirestore,
  SecretCheckResult,
} from "../secrets/tenantSecrets.js";

/**
 * Audit report for all tenants
 */
 ...
 */
async function runTenantSecretAudit(): Promise<AuditReport> {
  ...
  for (const tenantDoc of tenantsSnapshot.docs) {
    ...
    try {
      ...
      // Categorize result
      let status: "healthy" | "missing" | "placeholder" | "error";
...
export const auditSingleTenant = onRequest(
  ...
  async (req, res) => {
    const tenantId =
      req.query.tenantId || req.query.tenant_id || req.body?.tenantId || req.body?.tenant_id;
