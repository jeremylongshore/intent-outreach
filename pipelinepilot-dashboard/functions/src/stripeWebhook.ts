...
const API_SERVICES = ["CLAY", "APOLLO", "CLEARBIT", "CRUNCHBASE"] as const;
...
async function handleSubscriptionCreated(subscription: Stripe.Subscription): Promise<void> {
  ...
  for (const service of API_SERVICES) {
    ...
    try {
      ...
      const serviceKey = service.toLowerCase() as "clay" | "apollo" | "clearbit" | "crunchbase";
 ...
 */
async function createSecretStub(secretName: string, tenantId: string): Promise<void> {
 ...
 * @returns Plan name
 */
function mapPriceToPlan(priceId: string): "starter" | "pro" | "enterprise" {
